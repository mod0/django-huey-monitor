...
class ProcessInfoTestCase(TestCase):
    def test_linear_processing_task(self):
        ...
        with mock.patch.object(
            timezone, "now", MockDatetimeGenerator(offset=offset)
    ...
    def test_parallel_task(self):
        ...
        sub_tasks = TaskModel.objects.filter(parent_task=main_task_instance).order_by(
            "update_dt"
        )
        ...
        assert (
            sub_tasks1.human_progress_string() == "5/5it 100% 1.8 seconds/it finished"
        )
        ...
        assert (
            sub_tasks2.human_progress_string() == "5/5it 100% 1.8 seconds/it finished"
        )
